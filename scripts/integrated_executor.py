...
from scanner.websocket_monitor import HybridMonitor, DualFeedMonitor
from scanner.sequencer_feed import SequencedTransaction
from scanner.flashbots_relay import FlashbotsRelay, MultiRelayManager
from indexer.aave_indexer import AaveLightIndexer, HealthFactorEstimate
...
class IntegratedLiquidationExecutor:
    """
    High-performance liquidation executor with sequencer feed integration.

    Latency targets (with sequencer feed):
      - Sequencer tx detection:  ~30-80ms  (feed sees tx before block)
      ...
      - Total (feed-triggered):  ~130-230ms end-to-end

    Latency targets (Alchemy fallback):
    ...
    def __init__(
        ...
        # Light indexer (SQLite-based, sub-ms health factor estimates)
        self.indexer = None
        self.use_indexer = os.getenv("USE_LIGHT_INDEXER", "0") == "1"
        if self.use_indexer:
            self.indexer = AaveLightIndexer(
                w3=self.w3,
                aave_pool=self.chain.aave_pool,
                pool_data_provider=self.chain.aave_pool_data_provider,
                price_oracle=getattr(self.chain, 'price_oracle', self.chain.aave_oracle),
                db_path="/tmp/aave_indexer.db",
            )
            logger.info("Light indexer enabled ✅")
        else:
            logger.info("Light indexer disabled (set USE_LIGHT_INDEXER=1 to enable)")
    ...
    # ─── Sequencer Feed Early Warning ─────────────────────────

    async def on_liquidation_signal(self, tx: SequencedTransaction) -> None:
        """
        Called when the sequencer feed detects a liquidation-related transaction.
        This fires BEFORE the tx is included in a block — our early warning system.

        Strategy:
          1. Immediately check health factors of top at-risk borrowers
          2. If any are liquidatable, we have a race condition — execute NOW
          3. Otherwise, refresh tx cache for borrowers near threshold
        """
        logger.info(
            "🔔 Sequencer signal: liquidation tx detected seq=%d to=%s",
            tx.sequence_number, tx.to_address
        )

        # Emergency health check — only check borrowers already near threshold
        emergency_threshold = 1.02  # Tighter than normal 1.0
        at_risk = self.queue.get_at_risk(threshold=emergency_threshold, n=BATCH_SIZE)
        ...
        for addr, result in zip(addresses, health_results):
            ...
            # If liquidatable, assess immediately
            if hf < EXECUTE_HF_THRESHOLD and debt > 0:
                opp = await self._assess_opportunity(addr, hf, collateral, debt)
                if opp:
                    opportunities.append(opp)

        # Execute all viable opportunities immediately
        for opp in opportunities:
            self.opportunities_found += 1
            logger.info(
                "⚡ EMERGENCY EXECUTION triggered by sequencer feed "
                "for borrower %s (HF=%.4f)",
                opp.borrower, opp.health_factor
            )
    ...
    async def on_new_block(self, block: Dict[str, Any]) -> None:
        ...
        # 2. Health check — prefer indexer, fallback to Multicall3
        health_results = []
        indexer_hits = 0
        batch_addresses = []

        if self.use_indexer and self.indexer:
            for addr in addresses:
                est = self.indexer.estimate_health_factor(addr)
                if est and est.confidence in ("high", "medium"):
                    # Convert estimate back to batcher format:
                    # (collateral, debt, avail, ltv, liq_thresh, hf_raw)
                    # We don't have per-asset breakdown in estimate, so use totals
                    # This is approximate but sufficient for queue sorting
                    hf_raw = int(est.health_factor * 1e18)
                    health_results.append((0, int(est.total_debt_usd * 1e8), 0, 0, 0, hf_raw))
                    indexer_hits += 1
                else:
                    batch_addresses.append(addr)
                    health_results.append(None)  # placeholder
        else:
            batch_addresses = addresses

        if batch_addresses:
            batch_results = self.batcher.batch_health_factors(
                batch_addresses, self.chain.aave_pool
            )
            # Merge batch results back into health_results
            batch_idx = 0
            for i, addr in enumerate(addresses):
                if health_results[i] is None:
                    health_results[i] = batch_results[batch_idx]
                    batch_idx += 1

        self.health_checks += len(batch_addresses)
        if indexer_hits > 0:
            logger.debug("Indexer served %d/%d health checks", indexer_hits, len(addresses))
        ...
        # 6. Periodic borrower refresh + indexer sync
        if self.blocks_processed % REFRESH_INTERVAL == 0:
            asyncio.create_task(self._refresh_borrowers())
        
        # Sync indexer every 5 blocks when enabled
        if self.use_indexer and self.indexer and self.blocks_processed % 5 == 0:
            asyncio.create_task(self._sync_indexer())
    ...
    async def _sync_indexer(self) -> None:
        """Sync light indexer with latest Aave events."""
        if not self.indexer:
            return
        try:
            from_block = max(self.current_block - 10, 0)
            await self.indexer.sync_logs(from_block, self.current_block)
        except Exception as e:
            logger.warning("Indexer sync failed: %s", e)
    ...
    async def run(self) -> None:
        """Start the integrated executor with dual-feed monitoring."""
        logger.info("=" * 80)
        logger.info(" INTEGRATED LIQUIDATION EXECUTOR v2.1 — SEQUENCER FEED ENABLED")
        ...
        logger.info(" Feed: Dual (sequencer primary + Alchemy fallback)")
        logger.info("=" * 80)
        ...
        # Start dual-feed monitor
        http_url = os.getenv(self.chain.rpc_env_var)
        ws_url = os.getenv(self.chain.ws_env_var)
        monitor = DualFeedMonitor(
            alchemy_ws_url=ws_url,
            alchemy_http_url=http_url,
            monitored_contracts={self.chain.aave_pool},
        )
        monitor.on_block = lambda block: asyncio.create_task(self.on_new_block(block))
        monitor.on_liquidation_signal = lambda tx: asyncio.create_task(self.on_liquidation_signal(tx))
